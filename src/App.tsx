...
function getDishDescription(dish: Dish): string {
    ...
    if (nameLower.includes("пиво «бад 0»") || nameLower.includes("балтика 0") || (nameLower.includes("пиво") && nameLower.includes("безалкогольн"))) {
    ...
    if (nameLower.includes("коньяк")) {
        return "Благородный выдержанный коньяк глубокого янтарного оттенка с богатым дубовым букетом и долгим мягким послевкусием.";
    }

    // ── Виски ──
    if (nameLower.includes("виски")) {
        return "Классический купажированный виски с округлым пряным вкусом, нотками дубовой бочки и солода.";
    }

    // ── Вино и игристое ──
    if (nameLower.includes("вино") || nameLower.includes("шампанск") || nameLower.includes("игрист")) {
        return "Бутылочное вино с гармоничным вкусом и приятным фруктово-ягодным букетом, прекрасно дополняющее банкетный стол.";
    }

    // ── Коктейли и шоты ──
    if (dish.note) {
        if (nameLower.includes("шот")) return `Эффектный клубный шот от нашего бармена. Состав: ${dish.note}.`;
        ...
        if (nameLower.includes("лимонад")) return `Освежающий напиток со льдом и фруктовыми сиропами. Состав: ${dish.note}.`;
        return `Состав и ингредиенты: ${dish.note}.`;
    }
    if (nameLower.includes("лимонад")) return "Освежающий авторский прохладительный лимонад с цитрусовым соком и натуральным сиропом.";
    if (nameLower.includes("молочный коктейль")) return "Нежный густой молочный коктейль со сливочным мороженым и аппетитной пенкой.";
    if (nameLower.includes("сок")) return "Освежающий натуральный фруктовый сок в ассортименте.";
    if (nameLower.includes("кола")) return "Классический прохладительный газированный напиток.";
    ...
    if (nameLower.includes("горячий шоколад")) {
        return "Густой согревающий горячий шоколад с насыщенным сливочно-какао вкусом.";
    }
    if (nameLower.includes("гляссе")) {
        return "Ароматный кофе с шариком нежного сливочного мороженого и насыщенным вкусом.";
    }
    if (nameLower.includes("капучино")) {
        return "Свежеприготовленный кофе с пышной молочной пенкой и щепоткой ароматной корицы.";
    }
    ...
    // ── Фирменные соусы ──
    if (nameLower.includes("соус")) {
        return "Фирменный соус от шеф-повара, идеально подчеркивающий вкус мяса, шашлыка, пиццы и гарниров.";
    }
    ...
    if (nameLower.includes("картофель с чесноком") || nameLower.includes("запечённый картофель") || nameLower.includes("запеченный картофель")) {
    ...
    if (nameLower.includes("куриная грудка с овощами") || nameLower.includes("рагу овощное")) {
    ...
    if (nameLower.includes("удон")) {
        return "Японская пшеничная лапша удон в пряном соусе с овощами и кунжутом.";
    }
    if (nameLower.includes("жареные пельмени")) {
        return "Домашние пельмени, обжаренные до аппетитной хрустящей корочки со сметанным соусом.";
    }
    ...
    if (nameLower.includes("стейк из сёмги")) {
    ...
    if (nameLower.includes("пепперони")) {
        return "Фирменная пицца на тонком тесте с пикантной колбасой пепперони, томатным соусом и сыром моцарелла.";
    }
    ...
    // ── Салаты ──
    if (nameLower.includes("салат «белоснежка»")) {
    ...
    if (nameLower.includes("цезарь-ролл запечённый")) {
    ...
    if (nameLower.includes("палочки") || nameLower.includes("наггетс") || nameLower.includes("гренки") || nameLower.includes("кольца") || nameLower.includes("конвертики")) {
        return "Горячая аппетитная закуска во фритюре с золотистой хрустящей корочкой.";
    }
